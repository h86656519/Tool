...
public class CompanyView extends View {
    public static final String TAG = CompanyView.class.getSimpleName();
...
//        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.signature, 0, 0);
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.ActionBar, 0, 0);

        try {
//            backgroundColor = typedArray.getColor(R.styleable.signature_backgroundColor, context.getResources().getColor(R.color.white));
            backgroundColor = Color.WHITE;
//            penColor = typedArray.getColor(R.styleable.signature_penColor, context.getResources().getColor(R.color.penRoyalBlue));
            penColor = Color.BLACK;
//            penSize = typedArray.getDimension(R.styleable.signature_penSize, context.getResources().getDimension(R.dimen.pen_size));
            penSize = 30;
//            enableSignature = typedArray.getBoolean(R.styleable.signature_enableSignature, true);
    ...
    private void onTouchMoveEvent(float x, float y) {
        ...
        Line line = new Line(previousPoint.x, startPoint.y, x, y);
        line.setStartPoint(startPoint);
        line.setPreviousPoint(previousPoint);
        line.setCurrentPoint(currentPoint);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Log.i(TAG, "lines.size(): " + lines.size());
        for (int i = 0; i < lines.size(); i++) {
            drawMyLine(lines.get(i).lastWidth,
                    lines.get(i).currentWidth,
                    lines.get(i).previousPoint, //previousPoint
                    lines.get(i).startPoint, //startPoint
                    lines.get(i).currentPoint, //currentPoint
                    lines.get(i).velocity);
        }
    }
...
//        Line line = new Line(mid1.x, mid1.y, mid2.x, mid2.y);
//        lines.add(line);
    }

    private void drawMyLine(final float lastWidth, final float currentWidth
            , Point previousPoint, Point startPoint, Point currentPoint, final float velocity) {
